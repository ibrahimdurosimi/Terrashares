    ...
    <div className="relative">
      ...
      >
        {advantages.map((adv, idx) => (
          <div 
            key={idx} 
            className="snap-start shrink-0 w-[280px] md:w-[320px] bg-white dark:bg-[#0a0a0a] rounded-3xl p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-black/[0.03] dark:border-white/5 hover:-translate-y-1 transition-transform duration-300"
          >
            ...
            <h3 className="text-xl text-[#0A0A0A] dark:text-white mb-3" style={{ fontFamily: 'Georgia, serif' }}>{adv.title}</h3>
            <p className="text-[#0A0A0A]/60 dark:text-white/60 leading-relaxed">{adv.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-end gap-3 mt-4 pr-4">
        <button 
          onClick={scrollLeft}
          className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center text-[#0A0A0A] dark:text-white hover:bg-gray-50 dark:bg-white/5 transition-colors"
        >
